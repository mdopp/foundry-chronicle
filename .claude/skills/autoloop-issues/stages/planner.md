...
- **(a) Lint sweep** — opportunistic ruff/hygiene once #2 has established the toolchain. If `ruff check .` surfaces anything, `queue.py plan '{"id":…,"kind":"lint-sweep","file":…,"rule":…,"scope":…,"gate":…}'` per file/rule (skip files an open non-loop PR or non-blocked open issue touches).
- **(d) End-to-end validation** — only meaningful once #12 ships a deployable artifact. Until then this track does not exist; do not invent a substitute.
...
Evaluate this codebase across its areas: the service and its storage, the Foundry adapter, the web UI (note entry above all), the composition step, and — once they exist — the Discord recorder and the transcription stage.

Assume the baseline that this is a small, real, self-hosted tool used weekly by a handful of people, one instance per group. Do not give generic style-guide complaints unless they have a direct, measurable impact on bugs or developer velocity.
...
- **Secrets.** The Foundry token and Discord bot token in a log, a fixture, or a committed file.
