...
**What Foundry Chronicle is.** A standalone service that turns a tabletop RPG session into a readable chronicle. **One instance per group.** Two capture modes meet in one pipeline: an in-person session contributes only typed notes, an online session additionally contributes per-speaker Discord audio that is transcribed first — transcription is a *front stage*, not a second pipeline. Foundry VTT is a hard dependency and the source of every number (rolls, damage, loot); the language model orders text and inserts those facts, it never computes or invents them. Everything after capture runs in **batch**, so there is no latency budget and no GPU contention. Config is two things: Foundry URL + token, Discord bot token — everything else is pulled from Foundry. See the epic #1 for the three load-bearing decisions.

**Maturity note (2026-08).** The repo is greenfield: issues #2–#12 are the initial build-out. Until #2 lands there is no test suite and no CI, and until #12 lands there is no deployable artifact — so `stages/verify.md` describes a gate that does not exist yet. Read that file before assuming a real-environment verify is available.
...
- Worked examples of the same pipeline in sibling repos: `mdopp/servicebay` and `mdopp/solarisbay`. Both have a real-environment Verify stage; this repo will only get one with #12.
