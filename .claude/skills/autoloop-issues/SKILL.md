...
- CI: `.github/workflows/ci.yml` — gitleaks and the commit-subject check run on **every** PR; ruff and pytest + diff-coverage are path-filtered to Python/config paths. `build-images.yml` builds the service image on PR for image paths and publishes on `main`/tags. **A template-only / skill-only / docs-only PR runs only the unfiltered jobs** — no lint, no tests; for those the gate is local validation + real-box `/verify`.
